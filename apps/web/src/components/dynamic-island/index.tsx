...
interface BlobStateType {
  ...
  animationQueue: { size: SizePresets; delay: number }[];
  isAnimating: boolean;
}
...
type BlobAction =
  ...
  | {
      ...
      animationSteps: { size: SizePresets; delay: number }[];
    }
  | { type: "ANIMATION_END" };
...
interface DynamicIslandProviderProps {
  ...
  initialAnimation?: { size: SizePresets; delay: number }[];
}
...
const useScheduledAnimations = (
  animations: { size: SizePresets; delay: number }[],
) => {
...
const DynamicIslandContent = ({
  ...
  return (
    <motion.div
      id={id}
      className="mx-auto h-0 w-0 items-center justify-center border  bg-card text-center text-card-foreground transition duration-300 ease-in-out hover:shadow-md"
      animate={{
